...
if (typeof FBInstant !== 'undefined') {
  FBInstant.loading.setProgress(90);
}

// for some reason the viewport width isn't detected until a little later

// only start the game when FBInstant is defined
function waitUntilFBInstantIsLoaded() {
  if (typeof FBInstant !== 'undefined') {
    initializeGame();
  } else {
    setTimeout(function () {
      return waitUntilFBInstantIsLoaded();
    }, 100);
  }
}
...
function initializeGame() {
  ...
  // Wait a little bit to try to get the viewport dimensions
  document.addEventListener("DOMContentLoaded", function () {
    return setTimeout(function () {
      return initializeGameWithViewport();
    }, 1000);
  });
}
